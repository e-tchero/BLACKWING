...
pub enum RelayMessage {
    ...
    CandidateExchange {
        ...
        candidates: Vec<Candidate>,
        /// The 32-byte routing token generated by the relay for blind forwarding.
        relay_token: [u8; 32],
    },
    ...
    // ─── Phase 3: Relay Forwarding ───────────────────────────────────────────
    /// Sent by an endpoint to bind its current data-plane SocketAddr to the token.
    RelayEstablishRequest {
        /// The 16-byte intent identifier.
        intent_id: Vec<u8>,
        /// The requesting device's ID.
        device_id: DeviceId,
        /// Current timestamp in milliseconds (UNIX epoch) for replay prevention.
        timestamp: u64,
        /// Ed25519 signature over SHA-256(intent_id || device_id || timestamp).
        signature_bytes: Vec<u8>,
    },
    /// Acknowledgment from the relay that the binding was successfully updated.
    RelayEstablishAck {
        /// The intent identifier.
        intent_id: Vec<u8>,
    },
