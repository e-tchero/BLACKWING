...
impl EncodedFrame {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, &'static str> {
        ...
        let session_id = u64::from_be_bytes(
            data[0..8]
                .try_into()
                .map_err(|_| "slice error: session_id")?,
        );
        let sequence = u64::from_be_bytes(
            data[8..16]
                .try_into()
                .map_err(|_| "slice error: sequence")?,
        );
        let timestamp_us = u64::from_be_bytes(
            data[16..24]
                .try_into()
                .map_err(|_| "slice error: timestamp_us")?,
        );
        ...
        let width = u32::from_be_bytes(data[25..29].try_into().map_err(|_| "slice error: width")?);
        let height =
            u32::from_be_bytes(data[29..33].try_into().map_err(|_| "slice error: height")?);
        ...
        let payload_len = u32::from_be_bytes(
            data[34..38]
                .try_into()
                .map_err(|_| "slice error: payload_len")?,
        ) as usize;
